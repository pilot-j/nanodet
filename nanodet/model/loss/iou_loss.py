...
def eiou_loss(pred, target, eps=1e-7):
    r"""`Implementation of paper `Focal and Efficient IOU Loss for Accurate Bounding Box Regression
    ...
    w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1 + eps
    w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1 + eps
    del_w = ((w1-w2)/(cw+eps))**2
    del_h = ((h1-h2)/(cw+eps))**2
    left = ((b2_x1 + b2_x2) - (b1_x1 + b1_x2)) ** 2 / 4
    right = ((b2_y1 + b2_y2) - (b1_y1 + b1_y2)) ** 2 / 4
    rho2 = left + right
    ...
    # EIoU
    eious = ious - (rho2 / c2 + del_w +del_h)
    loss = (1 - eious) 
    return loss
